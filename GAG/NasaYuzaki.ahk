; NasaYuzaki's Modded Macro [SUMMER UPDATE]
...
searchItem(search := "nil") {
    ...
    if(search = "nil") {
        ...
        if (search = "recall") {
            ...
            uiUniversal("222115505544444444444444444444441111111155055", 1, 1)
        }
        else if (search = "pollen") {
            uiUniversal("22211550554444444444111111155055", 1, 1)
        }
...
/*
drawDebugBox(x, y, w := 10, h := 10) {
    Gui, DebugBox:Destroy
    Gui, DebugBox:+AlwaysOnTop +ToolWindow -Caption +LastFound +E0x20 +Owner
    Gui, DebugBox:Color, Red
    Gui, DebugBox:Show, x%x% y%y% w%w% h%h% NoActivate
    SetTimer, HideDebugBox, -500
}
...
eggItems := ["Common Egg", "Common Summer Egg", "Rare Summer Egg", "Mythical Egg", "Bee Egg", "Paradise Egg"
             , "Bug Egg"]

cosmeticItems := ["Cosmetic 1", "Cosmetic 2", "Cosmetic 3", "Cosmetic 4", "Cosmetic 5"
             , "Cosmetic 6",  "Cosmetic 7", "Cosmetic 8", "Cosmetic 9"]

honeyItems := ["Summer Seed Pack", "placeHolder1", "Delphinium Seed", "Lily Of The Valley Seed", "Traveler's Fruit Seed"
            , "Mutation Spray", "Oasis Crate", "placeHolder2", "Oasis Egg", "placeHolder3", "Hamster"]

realHoneyItems := ["Summer Seed Pack", "Delphinium Seed", "Lily Of The Valley Seed", "Traveler's Fruit Seed"
            , "Mutation Spray", "Oasis Crate", "Oasis Egg", "Hamster"]
...
ShowGui:
    ...
    Gui, Add, Tab, x10 y10 w580 h440 vMyTab, Seeds|Gears|Eggs|Event|Crafting|Cosmetics|Settings|Summer Harvest
    ...
    Gui, Add, GroupBox, x23 y50 w475 h340 ce8ac07, Honey/Summer Event Shop
    ...
    Gui, Add, Checkbox, % "x50 y140 vSelectAllHoney gHandleSelectAll cfad15f " . (SelectAllHoney ? "Checked" : ""), Select All Summer Event Items
    ...
    Gui, Add, GroupBox, x220 y200 w260 h110, Resolution
    Gui, Add, Text, x240 y220, Resolutions:
    IniRead, selectedResolution, %settingsFile%, Main, Resolution, 1
    Gui, Add, Radio, x240 y240 vselectedResolution gUpdateResolution c708090 %opt1%, 2560x1440 125`%
    Gui, Add, Radio, x240 y260 gUpdateResolution c708090 %opt2%, 2560x1440 100`%
    Gui, Add, Radio, x240 y280 gUpdateResolution c708090 %opt3%, 1920x1080 100`%
    Gui, Add, Radio, x240 y300 gUpdateResolution c708090 %opt4%, 1280x720 100`%
...
SetToolTip:
    ...
    if (rem60sec < 0)
        rem60sec := 0
    summerMin := rem60sec // 60
    summerSec := Mod(rem60sec, 60)
    ...
    if (selectedHoneyItems.Length()) {
        tooltipText .= "Summer Shop: " . honeyText . "`n"
    }
    if (AutoCollectPollinated) {
...
SeedShopPath:
    ...
    if (!shopOpened) {
        ...
        Gosub, alignment
        Sleep, 100
        ToolTip, re-Alignment Complete
        Sleep, 1000
        SendDiscordMessage(webhookURL, "Re-trying Honey Shop")
        Gosub, SeedShopPath
        Return
    }else{
...
HoneyShop:
    ...
    SendDiscordMessage(webhookURL, "**[Summer Shop Cycle]**")
    Sleep, 1000
    uiUniversal(11110)
    Sleep, 100
    uiUniversal(111110)
    ...
    ; checks for the shop opening up to 5 times to ensure it doesn't fail
    Loop, 5 {
        if (simpleDetect(0x1919E0, 10, 0.60, 0.20, 0.74, 0.325)) {
             shopOpened := true
    ...
    if (!shopOpened) {
        SendDiscordMessage(webhookURL, "Failed To Detect Summer Shop Opening [Error]" (PingSelected ? " <@" . discordUserID . ">" : "") )
        uiUniversal("63636362626263616161616363636262626361616161606561646056")
        Sleep, 100
        Gosub, alignment
        Sleep, 100
        ToolTip, re-Alignment Complete
        Sleep, 1000
        SendDiscordMessage(webhookURL, "Re-trying Summer Shop")
        Gosub, HoneyShop
        Return
    }else{
    ...
    hotbarController(0, 1, "0")
    SendDiscordMessage(webhookURL, "**[Summer Shop Completed]**")
    Sleep, 1000
    uiUniversal(11110)
...
F8::
{
    ; Edit this for Scanning Area -nasaYuzaki
    x1Ratio := 0.50
    y1Ratio := 0.20
    x2Ratio := 0.61
    y2Ratio := 0.325
    ...
    Gui, DebugOverlay:Destroy
    Gui, DebugOverlay:+AlwaysOnTop +ToolWindow -Caption +LastFound +E0x20
    Gui, DebugOverlay:Color, Red
    WinSet, Transparent, 100
    Gui, DebugOverlay:Show, x%x1% y%y1% w%width% h%height%, DebugRect
    SetTimer, RemoveDebugOverlay, -1000 ; remove overlay after 1 second
    return
}
